...
def build_text_record(origin_root: Path, path: Path) -> dict[str, Any]:
    ...
    st = path.stat()
    return {
        'id': object_id('rclone-local', rel),
        'title': rel,
        'text': text,
        'source': str(path),
        'tags': ['rclone', 'archive', 'local', 'rag'],
        'metadata': {
...
def build_binary_record(origin_root: Path, path: Path) -> dict[str, Any]:
    ...
    return {
        'id': object_id('rclone-local-meta', rel),
        'title': rel,
        'text': text,
        'source': str(path),
        'tags': ['rclone', 'archive', 'local', 'rag', 'metadata-only'],
        'metadata': {
            'origin_root': str(origin_root),
            'relative_path': rel,
            'source_kind': 'rclone-archive',
...
def main() -> None:
    ...
    ap.add_argument('--prune-prefix', default='rclone-local')
    args = ap.parse_args()
