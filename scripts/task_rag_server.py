#!/usr/bin/env python3
"""Lightweight RAG server for task retrieval on local.
...
WS = Path(os.environ.get('WORKSPACE', '/home/user/.appdir/workspace'))
SCRIPTS = WS / 'scripts'
...
class SearchReq(BaseModel):
    ...
    container: str = Field(default='host-z', min_length=1, description='Server-side container to search in.')
...
class ContainerReq(BaseModel):
    container: str = Field(default='host-z', min_length=1, description='Server-side container name.')
    timeout_s: int = Field(default=120, ge=1, le=600, description='Optional timeout in seconds for server-side script execution.')
