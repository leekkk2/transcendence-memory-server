# Verified main deployments to Configured host

`Deploy Configured host` runs after successful `CI/CD` on a same-repository push to main.
...
- `PRIMARY_NODE_HOST` variable: current public SSH address; overrides legacy `DEPLOY_HOST` secret.
- `DEPLOY_USER` / `DEPLOY_PORT`: default ubuntu / 22.
- Existing `DEPLOY_SSH_KEY`, `DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN` secrets.
- `deploy/primary-node.known_hosts` pins the host key under a stable HostKeyAlias.
- AWS Compose project: `tm-eva-restored-20260718`.
- Service directory: `/home/service/services/transcendence-memory-restore-20260718`.
