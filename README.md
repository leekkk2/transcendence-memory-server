...
- Ops helpers: `scripts/load_rag_config.sh`, `scripts/run_task_rag_server.sh`
  - `load_rag_config.sh` 从 `~/.appdir/workspace/tools/rag-config.json`（或 `RAG_CONFIG_FILE` 覆盖路径）导出 `RAG_ENDPOINT`、`RAG_AUTH_HEADER`、`RAG_API_KEY`、`RAG_DEFAULT_CONTAINER`
  - `run_task_rag_server.sh` 默认切到当前 server 仓库根目录执行，并将 `WORKSPACE` 默认指向该仓库根；只有在明确需要 monorepo 根级运行时才覆盖 `WORKSPACE`
- Nginx reference: `docs/nginx-memory.example.com.conf`
...
4. 准备运行时目录：`tasks/active`、`tasks/archived`、`tasks/rag/containers/host-z`、`memory`、`memory_archive`
...
- 私有部署链路当前口径：`memory.example.com` → Nginx (`docs/nginx-memory.example.com.conf`) → `127.0.0.1:8711` → `./scripts/run_task_rag_server.sh`
- 当前生产目标机器：local
- 现有 HTTPS 入口：`https://memory.example.com`
