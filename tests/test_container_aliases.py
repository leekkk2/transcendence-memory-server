...
def test_alias_crud_basic(tmp_path: Path):
    ...
    row = ca.upsert(
        alias="personal",
        canonical="personal-notes",
        reason="consolidate",
        status="active",
        notes="历史 12 obj 个人偏好",
    )
    assert row["alias"] == "personal"
    assert row["canonical"] == "personal-notes"
    ...
    got = ca.resolve("personal")
    assert got is not None
    assert got["canonical"] == "personal-notes"
...
def test_post_get_delete_aliases_admin(tmp_path: Path, monkeypatch):
    ...
    body = resp.json()
    assert body["alias"] == "personal"
    assert body["canonical"] == "personal-notes"
    ...
    body = resp.json()
    assert body["count"] == 1
    assert body["aliases"][0]["alias"] == "personal"

    # DELETE
    resp = client.delete("/containers/aliases/personal", headers=auth_headers())
    assert resp.status_code == 200
...
def test_ingest_objects_alias_routes_to_canonical(tmp_path: Path, monkeypatch):
    ...
    assert body["container"] == "personal"
    # 物理落盘是 canonical
    canonical_path = (
        workspace / "tasks" / "rag" / "containers" / "personal-notes" / "memory_objects.jsonl"
    )
    ...
    alias_dir = workspace / "tasks" / "rag" / "containers" / "personal"
    assert not (alias_dir / "memory_objects.jsonl").exists()


def test_ingest_objects_removed_alias_410(tmp_path: Path, monkeypatch):
    _, client = _build_client(tmp_path, monkeypatch)
    _seed_alias(client, alias="host-x", canonical="legacy-default", status="removed")
    ...
    assert detail["removed_alias"] == "host-x"


def test_canonical_still_works_unaffected(tmp_path: Path, monkeypatch):
    """客户端默认 container=legacy-default（self alias / canonical 自身）必须照常工作。

    这是 P0 验收硬标准 —— 不能因为 alias 层破坏现有客户端默认路径。
    """
    workspace, client = _build_client(tmp_path, monkeypatch)
    # 不注册 legacy-default alias，让它走"未命中 alias → name 本身是 canonical"路径
    resp = client.post(
        "/ingest-memory/objects",
        headers=auth_headers(),
        json={
            "container": "legacy-default",
            "auto_embed": False,
    ...
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["container"] == "legacy-default"
    canonical_path = (
        workspace / "tasks" / "rag" / "containers" / "legacy-default" / "memory_objects.jsonl"
    )
    assert canonical_path.exists()
...
def test_metadata_upsert_via_alias_writes_to_canonical(tmp_path: Path, monkeypatch):
    ...
    resp = client.post(
        "/containers/personal/metadata",
        headers=auth_headers(),
        json={"description": "via alias", "scope": "personal", "purpose": "active"},
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    # metadata 行的 name 字段是 canonical
    assert body["name"] == "personal-notes"
...
def test_dump_container_via_alias(tmp_path: Path, monkeypatch):
    ...
    _seed_alias(client, alias="personal", canonical="personal-notes")
    # 准备 canonical 数据
    root = workspace / "tasks" / "rag" / "containers" / "personal-notes"
    ...
    resp = client.get("/containers/personal/dump", headers=auth_headers())
    assert resp.status_code == 200
...
def test_containers_list_includes_aliases_field(tmp_path: Path, monkeypatch):
    ...
    _seed_alias(client, alias="personal", canonical="personal-notes", status="active")
    _seed_alias(client, alias="personal-old", canonical="personal-notes", status="deprecated")
    _seed_alias(client, alias="host-x", canonical="personal-notes", status="removed")
...
def test_search_single_container_via_alias_uses_canonical(tmp_path: Path, monkeypatch):
    ...
    resp = client.post(
        "/search",
        headers=auth_headers(),
        json={"container": "personal", "query": "foo", "topk": 3},
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    ...
    assert "personal-notes" in body["containers"]
    assert body["per_container_status"]["personal-notes"] == "not_initialized"
...
def test_search_removed_alias_410(tmp_path: Path, monkeypatch):
    ...
    resp = client.post(
        "/search",
        headers=auth_headers(),
        json={"container": "host-x", "query": "foo"},
    )
    assert resp.status_code == 410
...
def test_search_containers_array_resolves_each(tmp_path: Path, monkeypatch):
    ...
    _seed_alias(client, alias="personal", canonical="personal-notes")
    (workspace / "tasks" / "rag" / "containers" / "personal-notes").mkdir(parents=True)
    (workspace / "tasks" / "rag" / "containers" / "legacy-default").mkdir(parents=True)

    resp = client.post(
        "/search",
        headers=auth_headers(),
        json={"containers": ["personal", "legacy-default"], "query": "foo"},
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    # 两个目标都 resolve 完了
    assert set(body["containers"]) == {"personal-notes", "legacy-default"}
...
def test_delete_container_via_alias_400(tmp_path: Path, monkeypatch):
    ...
    resp = client.delete("/containers/personal", headers=auth_headers())
    assert resp.status_code == 400
    detail = resp.json()["detail"]
    assert detail["error"] == "cannot_delete_via_alias"
    assert detail["canonical"] == "personal-notes"
    # canonical 数据未被破坏
    assert (workspace / "tasks" / "rag" / "containers" / "personal-notes").exists()
...
def test_delete_container_canonical_still_works(tmp_path: Path, monkeypatch):
    ...
    resp = client.delete("/containers/personal-notes", headers=auth_headers())
    assert resp.status_code == 200
    assert not (workspace / "tasks" / "rag" / "containers" / "personal-notes").exists()
...
def test_container_index_status_via_alias(tmp_path: Path, monkeypatch):
    ...
    resp = client.get(
        "/containers/personal/index-status", headers=auth_headers()
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
