...
def test_metadata_crud_basic(tmp_path: Path):
    ...
    assert row["tags"] == ["a", "b"]
    assert row["scope"] == "yzjx"
    assert row["entity"] == "egonex"
...
def test_metadata_partial_upsert_preserves_fields(tmp_path: Path):
    ...
    cm = ContainerMetadata(str(tmp_path / "lancedb"))
    cm.upsert("foo", description="v1", tags=["a"], scope="yzjx")
...
def test_post_metadata_upsert_200(tmp_path: Path, monkeypatch):
    ...
    payload = {
        "description": "Egonex eng container",
        "tags": ["egonex", "engineering"],
        "scope": "yzjx",
        "entity": "egonex",
        "purpose": "eng",
        "owner": "claude-on-mac",
    ...
    resp = client.post(
        "/containers/yzjx-egonex-eng/metadata",
        headers=auth_headers(),
        json=payload,
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["name"] == "yzjx-egonex-eng"
    assert body["description"] == "Egonex eng container"
    assert body["tags"] == ["egonex", "engineering"]
    assert body["scope"] == "yzjx"
    assert body["entity"] == "egonex"
