...
- torch 变体:CI 构建发布的 base 用 **CPU wheel**(`--build-arg TORCH_INDEX_URL=https://download.pytorch.org/whl/cpu`),因部署主机(eva)无 GPU,剔除 ~4GB CUDA 库(见 DR-048 @ memory-app)。OSS 用户自建默认走常规(GPU 友好),详 onboarding §3。
...
- 各自只新增**薄代码层**(MB 级)。
- 无 base 时 2 服务 ≈ 2×镜像全量;有共享 base ≈ 1×base + 2×薄 diff。**eva 实测净省 ~4-5GB**。
