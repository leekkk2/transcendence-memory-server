...
| Reranker upstream returns 400 on `/v1/rerank` (`model_price_error` / `model_not_found`) | OpenAI-compat aggregator gateways (newapi / one-api / litellm) only honor model ids actually registered as channels. A fabricated rerank id (`text-reranker`, `selfhosted-bge` as model name, etc.) is rejected even if the gateway exposes a `/v1/rerank` endpoint. | (a) If you have a real rerank channel registered (Cohere, BGE, Jina): set `rerankers[].model` to that channel's id (e.g. `rerank-v3.5`, `bge-reranker-v2-m3`). (b) If only embedding channels are registered: set `model: text-embedding-3-small` — the gateway routes `/v1/rerank` to embedding-cosine pseudo-rerank, returning valid Cohere-v2 schema. Verify with `curl POST /v1/rerank -d '{"model":"text-embedding-3-small","query":"x","documents":["a","b"],"top_n":2}'`. Quality bounds: see `docs/operations/rerank-perf-baseline.md`. |
