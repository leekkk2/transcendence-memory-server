...
| elapsed | 6.2m |
| batches | 58 (size 100) |
| old table backup | `tasks/rag/containers/host-z/lancedb/chunks_old_20260516_153012.lance` |
| new active table | `tasks/rag/containers/host-z/lancedb/chunks.lance` |

Old table preserved — verify search results, then remove manually:
`rm -rf tasks/rag/containers/host-z/lancedb/chunks_old_20260516_153012.lance`
```
...
```bash
cd tasks/rag/containers/host-z/lancedb
...
```bash
for c in host-z team test-alpha; do
    python scripts/migrate_embeddings.py \
        --container "$c" \
        --from gemini-3072 \
        --to   openai-small-1024 \
