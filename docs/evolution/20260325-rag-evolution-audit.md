...
> 目的：把 **git 仓库未完整记录**、但已实际发生在 local / OpenClaw 工作区 / 相关会话中的服务端演进过程补整理出来，方便回看它是怎么一步步收敛到当前 `transcendence-memory-server` 形态的。

## 0. 审计范围

本次整理来源包括：

1. 当前仓库：`~/.appdir/rag-everything`
2. local 工作区现状：`~/.appdir/workspace/scripts/*task_rag*`
3. 工作区记忆：`memory/*.md`
...
- README 中已经明确：
  - 统一部署在 local
  - host-z / local / Aliyun 共用
  - Aliyun 不本地安装

也就是说：**仓库里保留了“最早能跑通的 local 中心化版本”，但没有后续几轮大改。**

---

## 2. 第一阶段：从任务记忆 / RAG-Anything 想法，落到 local 中心化 RAG 脚手架

从工作区记忆 `memory-host-z/2026-03-18.md` 与相关 RAG 容器数据可回收出以下阶段事实：

### 已发生的关键决策

- 用户批准将 RAG 方案落到 local 上统一部署。
- 明确要求：
  - **RAG-everything 统一部署在 local**
  - **供 host-z / local / Aliyun 共用**
  - **Aliyun 不本地安装**

### 当时已落地的内容

- 创建任务卡：`TASK-20260318-001-rag-anything-local-openclaw`
- 初版脚手架已成型：
...
- 过渡到“可部署的 local 中心化 task RAG 服务”
- 还没有进入 LanceDB-only 架构

---

## 3. 第二阶段：服务化上线到 local（8711 / Nginx / 域名 / systemd）

后续从工作区文档、技能资料和系统侧痕迹可以确认，服务化部署已完成：

### 服务与网关层

- 正式域名：`https://memory.example.com`
- 内部端口：`127.0.0.1:8711`
- systemd 服务：`rag-everything.service`
- Nginx 反代：`memory.example.com -> 127.0.0.1:8711`
- TLS：Let’s Encrypt / Certbot

### 可回收证据

- `/etc/systemd/system/rag-everything.service`
- `/etc/nginx/sites-available/memory.example.com`
- `/etc/letsencrypt/renewal/memory.example.com.conf`
- 工作区技能引用文档：
...
这一步说明 RAG-everything 不再只是本地脚本集合，而是已经成为 **local 上正式对外提供的记忆检索服务**。
...
来自 `memory/2026-03-22-task-coordination.md`、`memory/2026-03-23.md`、`memory/2026-03-23-rag-cleanup.md`、`docs/ops/rag-local-backend-handover-20260323.md`：
...
- `local` 容器完成过 LanceDB 重建
- `host-z` 容器完成过 LanceDB 重建
- 文档中多次强调：
  - **前端理论上可无感使用**
...
从 `memory/2026-03-22-docker-sysctl.md`、`docs/ops/rag-local-embed-repair.md`、`memory/2026-03-23.md` 可回收出一段很关键的“线上事故演化”。
...
从 `memory/2026-03-24.md`、`docs/ops/20260324-local-TO-ALL-RESULT-RAG-SKILL-INTEGRATED-AND-WORKSPACE-INGESTED.md` 可确认：

### 已完成事项

- 安装并接入 `skills/rag-everything-enhancer`
- 对齐工作区 RAG 配置：
  - `tools/rag-config.json` -> `https://memory.example.com`
  - 默认容器 -> `local`
...
- `docs/ops/20260324-local-TO-ALL-RESULT-RAG-GENERIC-STRUCTURED-INGEST-IMPLEMENTED.md`
...
3. **某些修复是否先发生在其他机器 / 会话后再同步到 local**
   - 例如 host-z / Aliyun 参与的那部分，仅能恢复“结果与结论”，未必能精确还原全部操作顺序
...
1. **RAG-Anything / 任务记忆增强构想落地为 local 中心化 RAG**
...
> 这个项目已经不是“最初那个小型 task RAG repo”了，而是演变成了 **local 上统一提供给多设备、多工作流使用的 RAG 服务与增强层**。
